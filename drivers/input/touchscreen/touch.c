	...
	case 20701:
		...
		if (strstr(tp_ic_name, "ili9882n") && strstr(saved_command_line, "ili9882n")) {
			return true;
                ...
                if (strstr(tp_ic_name, "focaltech,ft8722") && strstr(saved_command_line, "ft8722")) {
                        return true;
                }
		pr_err("[TP] ERROR! ic is not match driver\n");
		return false;
...
//   panel_data->tp_type = g_tp_dev_vendor;
	...
	if (strstr(saved_command_line, "hx83112a_txd_boe_fives")) {
		...
		hw_res->RX_NUM = 36;
		vendor = "TXD";
	} else if (strstr(saved_command_line, "ft8722_hlt_boe_eighth")) {
		hw_res->TX_NUM = 18;
		hw_res->RX_NUM = 36;
		vendor = "HLT";
	}
	...
	if (g_tp_prj_id == 20701) {
		...
		if (strstr(saved_command_line, "ft8006s_hlt_boe")) {
			...
			panel_data->firmware_headfile.firmware_size = sizeof(FW_FT8006S_NF_90HZ_HLT);
                ...
                if (strstr(saved_command_line, "ft8006saa_truly_truly")) {
                        snprintf(panel_data->fw_name, MAX_FW_NAME_LENGTH,
                                "tp/20701/FW_FT8006S_NF_90HZ_TRULY.img");
                        snprintf(panel_data->test_limit_name, MAX_LIMIT_DATA_LENGTH,
                                "tp/20701/LIMIT_FT8006S_NF_90HZ_TRULY.img");
                ...
                if (strstr(saved_command_line, "ft8722_hlt_boe_eighth")) {
                        snprintf(panel_data->fw_name, MAX_FW_NAME_LENGTH,
                                "tp/20701/FW_FT8722_NF_90HZ_HLT.img");
                        snprintf(panel_data->test_limit_name, MAX_LIMIT_DATA_LENGTH,
                                "tp/20701/LIMIT_FT8722_NF_90HZ_HLT.img");
