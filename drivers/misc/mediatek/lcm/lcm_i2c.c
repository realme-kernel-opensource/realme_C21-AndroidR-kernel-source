...
static int _lcm_i2c_write_bytes(unsigned char addr, unsigned char value)
{
	...
	if (ret < 0)
		pr_err("[LCM][ERROR] _lcm_i2c write data fail ret=%d!!\n", ret);
...
int rc=0;
	pr_err("%s\n", __func__);
	if (is_project(20701)) {
		pr_info("%s,config _lcm_i2c_bias nt5038\n", __func__);
		if (0x02 == _lcm_i2c_write_bytes(0x00, voltage_value_offset)) {
			if (0x02 == _lcm_i2c_write_bytes(0x01, voltage_value_offset)) {
				if (0x02 == _lcm_i2c_write_bytes(0x03, 0x73)) {
					return 0;
				}
			}
		}
		pr_err("oops! [LCD] gate ic setting nt5038 error \n");
		return -2;
	}
	else {
...
{
	if (is_project(20701)) {
		printk("_lcm_parse_bias riod\n");
		strcpy(_lcm_i2c_of_match->compatible,"LCD_BIAS_NT5038");
	}
	else {
		if (!arg)
			return -EINVAL;
